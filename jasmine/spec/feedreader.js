...
$(function () {
    ...
    */
    describe('RSS Feeds', function () {
         ...
         */
        it('all feeds has a defined URL and it is not empty', () => {
         ...
         */
        it('all feeds has a defined name and it is not empty', ()=> {
            for (let feed = 0; feed < allFeeds.length; feed++) {
    ...
    describe('The menu', function () {
        ...
        it('the menu changes visibility when the menu icon is clicked', ()=> {
            ...
            expect(document.querySelector('body').classList.contains('menu-hidden')).toEqual(false);
            $('.menu-icon-link').trigger("click");
            expect(document.querySelector('body').classList.contains('menu-hidden')).toEqual(true);
        });
    });
    ...
    describe('New Feed Selection', function () {
         ...
         */
        let intialFeed, finalFeed;
        beforeEach(function (done) {
            loadFeed(0, () => {
                intialFeed = $('.feed').innerText;
                loadFeed(1, () => {
                    finalFeed = $('.feed').innerText;
                    done();
                });
            });
        });
        ...
        it('when a new feed is loaded, the content actually changes', (done) => {
            expect(intialFeed).not.toEqual(finalFeed);
            done();
        });
    });
}());
